...
class BasicUNet25D(nn.Module):
    def __init__(
        ...
        self.conv_0 = TwoConv(dimensions, in_channels, features[0], act, norm, dropout)
        self.down_1 = Down(dimensions, fea[0], fea[1], act, norm, dropout, pool_size=(1, 2, 2))
        self.down_2 = Down(dimensions, fea[1], fea[2], act, norm, dropout, pool_size=(1, 2, 2))
        self.down_3 = Down(dimensions, fea[2], fea[3], act, norm, dropout, pool_size=(1, 2, 2))
        self.down_4 = Down(dimensions, fea[3], fea[4], act, norm, dropout, pool_size=(1, 2, 2))
        self.drop = nn.Dropout()
        self.upcat_4 = UpCat(dimensions, fea[4], fea[3], fea[3], act, norm, dropout, upsample, pool_size=(1, 2, 2))
        self.upcat_3 = UpCat(dimensions, fea[3], fea[2], fea[2], act, norm, dropout, upsample, pool_size=(1, 2, 2))
        self.upcat_2 = UpCat(dimensions, fea[2], fea[1], fea[1], act, norm, dropout, upsample, pool_size=(1, 2, 2))
        self.upcat_1 = UpCat(dimensions, fea[1], fea[0], fea[5], act, norm, dropout, upsample, halves=False, pool_size=(1, 2, 2))
...
class BasicUNetUncer(nn.Module):
    def __init__(
            self,
            dimensions: int = 3,
            in_channels: int = 1,
    ...
    def uncer(self, x: torch.Tensor):
        ...
        logits = self.final_conv(u1)
        uncer_out = self.drop(logits).mean(dim=1)
        uncer_out = torch.sigmoid(uncer_out)
