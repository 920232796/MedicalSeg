...
from medical_seg.networks.nets.transformer.transformer_seg import Transformer
from medical_seg.networks.nets.transformer.transformer_model import TransConfig
...
class BasicUNetTrans(nn.Module):
    def __init__(
        self,
        dimensions: int = 3,
        in_channels: int = 1,
        ...
        config = TransConfig(in_channels=fea[4], out_channels=fea[4], hidden_size=hidden_size,
                                num_attention_heads=num_attention_head, num_hidden_layers=num_hidden_layer)
        self.encoder_trans = Transformer(config)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x3 = self.down_3(x2)
        x4 = self.down_4(x3)
        x5 = self.encoder_trans(x4)
        # x4 = x5 + x4
        # x4 = torch.cat([x4, x5], dim=1)
        u4 = self.upcat_4(x5, x3)
